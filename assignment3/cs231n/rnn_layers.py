...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    for time in range(T):
      next_h, cache_ = rnn_step_forward(x[:,time,:], prev_h, Wx, Wh, b)
      h[:,time,:] = next_h
      cache.append(cache_)
...
def rnn_backward(dh, cache):
    ...
    for time in reversed(range(T)):
      cache_ = cache[time]
      dh_sum = dh[:,time,:]+dnext_h
      dx_,dprev_h,dWx_,dWh_,db_ = rnn_step_backward(dh_sum , cache_)
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    #############################################################################
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    i_g,f_g,o_g,g_g = np.split(prev_h.dot(Wh) + x.dot(Wx)+b, 4, axis =1 )
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    do_g = dnext_h * np.tanh(next_c) * o*(1-o)
    df_g = dnext_c * prev_c * f*(1-f)
    di_g = dnext_c * g *i*(1-i)
    dg_g = dnext_c * i * (1 - g**2)
    
    dsum = np.concatenate((di_g, df_g, do_g, dg_g), axis=1)
    dx = dsum.dot(Wx.T)
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for time in range(T):
      next_h, next_c, cache_ = lstm_step_forward(x[:,time,:], prev_h, prev_c, Wx, Wh, b)
      h[:,time,:] = next_h
      cache.append(cache_)
...
def lstm_backward(dh, cache):
    ...
    for time in reversed(range(T)):
      cache_ = cache[time]
      dh_sum = dh[:,time,:]+dnext_h
      dx_,dprev_h,dprev_c,dWx_,dWh_,db_ = lstm_step_backward(dh_sum, dnext_c, cache_)

...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        pos_captions = self.positional_encoding(embed_captions) #N,T,W
        proj_features = self.visual_projection(features).unsqueeze(1) #N,1,W
        
        tgt_mask = torch.tril(torch.ones(T, T)) #삼각행렬 생성

        features = self.transformer(tgt=pos_captions, 
                                    memory=proj_features, 
                                    tgt_mask=tgt_mask)

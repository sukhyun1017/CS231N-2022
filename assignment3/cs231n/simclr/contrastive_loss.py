...
def sim(z_i, z_j):
    ...
    norm_product = torch.linalg.norm(z_j)*torch.linalg.norm(z_i)
    norm_dot_product = torch.matmul(z_j,z_i)/ norm_product
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        second_sum = 0
        for i in range(2*N):
          first_sum += torch.exp(sim(z_k,out[i])/tau)
          second_sum += torch.exp(sim(z_k_N,out[i])/tau)
        first_sum -= torch.exp(sim(z_k,z_k)/tau)
        second_sum -= torch.exp(sim(z_k_N,z_k_N)/tau)

        first_loss = -torch.log(torch.exp(sim(z_k,z_k_N)/tau)/first_sum)
        second_loss = -torch.log(torch.exp(sim(z_k_N,z_k)/tau)/second_sum)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    norm_product = torch.linalg.norm(out_left,dim=1)*torch.linalg.norm(out_right,dim=1)
    pos_pairs = torch.sum(out_left*out_right, dim = 1) /norm_product
    pos_pairs = torch.unsqueeze(pos_pairs,1)
    pass
...
def compute_sim_matrix(out):
    ...
    norm = torch.linalg.norm(out, dim=1).unsqueeze(1)
    norm_product = norm*norm.T
    sim_matrix = dot/norm_product
    pass
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    numerator = None
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    numer = torch.exp(sim_pos/tau)
    numerator = torch.cat([numer, numer], dim=0)
    pass

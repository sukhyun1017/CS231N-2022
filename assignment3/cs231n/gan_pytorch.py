...
def discriminator_loss(logits_real, logits_fake):
    ...
    true_labels = torch.ones(len(logits_real)).type(dtype)
    false_labels = torch.zeros(len(logits_fake)).type(dtype)
...
def generator_loss(logits_fake):
    ...
    true_labels = torch.ones(len(logits_fake)).type(dtype)
...
def build_dc_classifier(batch_size):
    ...
    model = nn.Sequential(
            ...
            nn.Conv2d(1,32,5, stride = 1), #(32,24,24)
            nn.LeakyReLU(0.01),
            nn.MaxPool2d(2,stride = 2),    #(32,12,12)
            
            nn.Conv2d(32,64,5, stride = 1),#(64,8,8)
            nn.LeakyReLU(0.01),
...
def run_a_gan(D, G, D_solver, G_solver, discriminator_loss, generator_loss, loader_train, show_every=250,
    ...
    for epoch in range(num_epochs):
        for x, _ in loader_train:
            ...
            gen_logits_fake = D(fake_images.view(batch_size, 1, 28, 28)) #logits_fake를 사용안헀네

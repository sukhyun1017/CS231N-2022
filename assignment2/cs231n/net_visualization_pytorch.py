...
def compute_saliency_maps(X, y, model):
    ...
    scores = model(X)
    scores = scores.gather(1,y.view(-1,1)).squeeze().sum()
    scores.backward()

    saliency = X.grad.abs().max(axis=1)[0]
    pass
...
def class_visualization_update_step(img, model, target_y, l2_reg, learning_rate):
    ...
    score = pred_y[0, target_y] - l2_reg * torch.square(torch.norm(img))
    score.backward()
